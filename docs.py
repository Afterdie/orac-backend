...
def gen_docs(connection_string:Optional[str], localSchema: Optional[dict]):
    #local db gen
    if not connection_string:
        if not localSchema:
            return {"success":False, "message": "Local schema not provided"}
        prompt = f"""
    ...
    if not localSchema:
        if not connection_string or not isinstance(connection_string, str):
            return {"success":False, "message": "Connection string not provided"}
        #system prompt
    ...
    try:
        ...
        return {"success": True, "blocks": cleaned_json["blocks"]}
